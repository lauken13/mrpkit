...
#' values = list("cat" = "cat", "kitten" = "cat","dog" = "dog","dog" = "puppy"))
...
question <- R6::R6Class(
  ...
  list(
    ...
    initialize = function(name,col_names,values_map) {
      ...
      if(sum(duplicated(self$values[,2]))>0 | sum(duplicated(self$values[,1]))>0) {
    ...
    print = function(...) {
      ...
      for(j in 1:nrow(self$values)){
          cat(as.character(self$values[j,1]), "=", as.character(self$values[j,2]), '\n')
      }
      invisible(self)
    }
  )
