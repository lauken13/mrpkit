...
Some of the methods in MRP-Kit use external packages i.e. `forcats` (Wickham, 2020) in `SurveyMapping$mapping`, `dplyr` (Wickham, François, Henry & Müller, 2020) in  `SurveyMapping$tabulate`, and `ggplot2` (Wickham, 2016) in `SurveyFit$plot`. To fit the MRP model, you might also need these packages:
...
shape_pref <- SurveyData$new(
  ...
  questions = list(age = "Please identify your age group",
                ...
                y = "If today is election day, will you vote for the shape?"),
...
q2 <- SurveyQuestion$new(
 ...
 values_map = list("Neverland Labor Party" = "shape", "shape" = "shape", "Neverland Democrats" = "The Democrats", "The Democrats" = "The Democrats"))
...
The `SurveyMap` object holds the mapping between a set of items in the survey and post-stratification datasets. It takes the `SurveyData` objects, which in this case are `shape_pref` and `popn_obj`, together with labels and values that have been matched and provided in the `SurveyQuestion` object. The mapped object would specify the correspondences in the variables that will be used when fitting the model. 
...
ex_map <- SurveyMap$new(sample = shape_pref, population = popn_obj, q1)
...
```{r predict}
# predict the probability of voting for the shape using the fit1 model
poststrat_fit1 <- fit1$population_predict()
```
...
This method then will return a dataframe with variable levels, the draw/iteration, and the probability of the `y`. In this example, `age_preds` represents the age-group, the draw, and the probability of voting for the shape party for each age-group and each draw. 
...
Once we have the aggregated value of the estimates, we are able to visualize it using the `plot` method. This method takes the aggregated data frame as the argument. It generates a violin plot of the estimated aggregated variable or the population prediction. In the example below, we show the example of the estimate of `age` and `state` variables. 

```{r age-vis}
plot_age <- fit1$plot(age_preds) 
plot_age
```

The plot above shows the distribution of shape support for each age-group. We can see that the probability is normally distributed in each age-group. The plot also implies that people who are aged 56-65 are most likely to vote for the shape.

The next plot displays the probability of voting for the shape by state. We learn that the probability of the shape winning the election is high since the majority of people (around 75%) in all of the states in Neverland vote for that party. 


```{r state-vis}

plot_state <- fit1$plot(state_preds)
plot_state
