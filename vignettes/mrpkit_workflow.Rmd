...
This object also has three methods, which are `SurveyData$new`, `SurveyData$print`, and `SurveyData$clone`. The `SurveyData$new` is used to transform a survey data frame to a `SurveyData` object, whereas `SurveyData$print` and `SurveyData$clone` are used to print and clone the created `SurveyData` object. 
...
nlp_pref <- SurveyData$new(
  ...
  questions = c("Please identify your age group",
                ...
                "If today is the election day, will you vote for the NLP party?"),
...
nlp_pref$print()
popn_obj$print() #put this issue to github
```
