...
nlp_pref <- SurveyData$new(
  nlp_survey[,c("age","gender","vote_for", "highest_educ", "state", "y")],
  questions = c("Please identify your age group",
                "Please select your gender",
                "Which party did you vote for in the 2018 election?",
                "Please identify your completed highest education",
  ...
  responses = list(levels(nlp_survey$age),
                   ...
                   levels(nlp_survey$highest_educ),
                   levels(nlp_survey$state),
                   c("no","yes")),
...
popn_obj <- SurveyData$new(
  approx_voters_popn[,c("age_group","gender","vote_pref", "education", "state")],
...
These column labels and levels should be aligned to be able to perform MRP. With `SurveyQuestion$new` method, we are able to define the same name of column label and mapping its value into the same level. Further, if there is a meaningful ordering over the values, they should be sorted over that order, either descending or ascending. 
...
# create SurveyQuestion$object for question related to education
...
# create SurveyQuestion$object for question related to state
...
4. `delete`
    ...
    Now we already have all of the variables in the post-stratification data mapped. Sometimes, you do not want including all of the variables in the post-stratification matrix, for example, in this case, we want to exclude `party_pref` from the matrix using `delete` method.
...
5. `replace`
    ...
    Suppose that you changed your mind and want to use another level matching for a certain question. For example, in question regarding education, q4, you want to change the "associates" level to be equal to "4-years college". To do this, you can firstly create a new `SurveyQuestion` object corresponding to that new level mapping. Secondly, you can use `replace` method to change the question object. This method takes two arguments, namely `old question` as the first argument and the `new question` as the second argument. The example of these steps could be seen as follows:
...
Furthermore, if the model fitting done with one of the built-in function (`rstanarm::stan_glmer`, `lme4::glmer`, `brms::brm`), then this method does not take any arguments. However, if you used a customized function in the model fitting, then this method takes two argument. The first is the fitted model object, and the second is the post-stratification data frame. In this example, we will show how to use this method for `mod_fit_1` object. 

```{r predict}
# predict the probability of voting the NLP using the fit1 model
...
The next step is to generate the population estimate or group estimate using `aggregate` method. This method takes two arguments, namely the post-stratification estimate data and the variable that is aggregated, in this example we want to aggregate `age` and `state` variable. If the variable is not specified, then this method would generate a population estimate. 
...
This method then will return a data frame with variable level, the draw/iteration, and the probability of the `y`. In this example, `age_preds` represents the age level, the draw, and the probability of voting the NLP party for each age level and each draw. 
...
Once we have the aggregated value of the estimates, we are able to visualize it using `visify` method. This method takes in the aggregated data frame as the argument. It generates a violin plot of the estimate aggregated variable or the population prediction. In the example below, we show the example of the estimate of `age` and `state` variable. 
...
The plot above shows the distribution the NLP votes probability for each age group. We can see that the probability is normally distributed in each age level. The plot also implies that people who are age 56-65 are most likely to vote for the NLP. 

The next plot, which is shown below, displays the probability of voting the NLP by state. We learn that the probability of the NLP win the election is pretty high since majority of people (around 75%) in all of the states in Neverland vote that party. 
