...
4. `delete`
    ...
    Now we already have all of the variables in the post-stratification data mapped. Sometimes, you do not want including all of the variables in the model, for example, in this example, we want to exclude `gender` from the model. **Note:Not including in the model, but in post-stratification matrix**
...
The next step is to generate the population estimate or group estimate using `aggregate` method. This method takes two arguments, namely the post-stratification estimate data and the variable that is aggregated, in this example we want to aggregate `age` variable. If the variable is not specified, then this method would generate a population estimate. 

```{r eval = FALSE}

##### the name of this method would be changed
