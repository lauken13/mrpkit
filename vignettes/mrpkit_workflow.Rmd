...
1. Prepare the data.
2. Create `SurveyData` objects for the survey and post-stratification datasets.
3. Create a `SurveyMap` object to match the values between the survey and post-stratification datasets for each question.
...
Note that this method will automatically use each of the factor, character, and binary variables to create questions and responses if you do not specify the list of questions and responses. After `SurveyData` is initialized, it can be displayed in the console using the `print` method.
...
The `SurveyMap` object has `new`, `add`, `delete`, `replace`, `mapping`, `tabulate`, and `fit` methods.
...
In this stage, we already have the fitted model object, i.e., `fit1` and `fit2`. These objects have been automatically stored as `SurveyFit` objects. Using this object we can generate the predicted probabilities for the post-stratification cells, and aggregate the small area estimation as needed, for example, state or province estimates, and visualize the aggregated estimates. 
...
The next plot displays the probability of voting for the BP by the state without using the weight. We learn that the probability of the BP winning the election is high since most people in almost all the states (around 75%) vote for that party. State B is the state with the lowest probability of vote for the BP. The distribution still ranged more than 50%. Note that this violin plot is generated using `ggplot2` (Wickham, 2016). Hence, it could be modified the same way `ggplot2`'s plot is modified. For example, here, we can add the title and change the theme of the plot.
