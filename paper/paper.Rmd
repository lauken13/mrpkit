...
The MRP workflow implemented by `mrpkit` is summarised in Figure \@ref(fig:illustrate). In this section, these steps are expanded on. At each of them there are a variety of decisions that must be made. The `mrpkit` package makes each of these explicit.
...
## Finalising the mapping and tabulation
...
The first step is to gather the datasets. For the survey data we are interested in the Nationscape dateset, which can be requested from the Voter Study Group. Access is usually granted within hours and is completely free. We are going to use the survey from 25 June 2020.
...
# 
# nationscape_data$stateicp %>% unique()
# 
# # Replace NAs with DC
...
raw_poststrat_data$stateicp <- as.character(raw_poststrat_data$stateicp)
...
#       )
  ...
  filter(!stateicp %in% c("puerto rico", "state groupings (1980 urban/rural sample)", "military/mil. reservations", "state not identified"))
