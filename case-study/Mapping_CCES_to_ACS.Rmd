...
cces_2018_subset <- droplevels(cces_2018_subset)  # remove unused categories
```
...
acs_2018_subset_tbl <- read_ipums_micro(acs_2018_subset_ddi)
# save as R objects
save(acs_2018_subset_tbl, acs_2018_subset_ddi, file= "acs_2018_subset.RData")
```
...
str(acs_2018_subset_tbl, max.level=2, list.len=6)
dim(acs_2018_subset_tbl)
names(acs_2018_subset_tbl)
```
...
```{r}
length(table(acs_2018_subset_tbl$AGE))
ipums_val_labels(acs_2018_subset_tbl$AGE)
```

The function [`as_factor`](https://haven.tidyverse.org/reference/as_factor.html)
allows us to treat labelled vectors as factors.
The function [`zap_labels`](https://haven.tidyverse.org/reference/zap_labels.html)
allows us to treat labelled vectors as vectors.
See https://haven.tidyverse.org/articles/semantics.html for further information.
```{r}
head(table(acs_2018_subset_tbl$AGE))
head(table(as_factor(acs_2018_subset_tbl$AGE)))
head(table(zap_labels(acs_2018_subset_tbl$AGE)))
```
...
We would like to poststratify to a representative survey population,
i.e., the ACS micro-census, but that subset of the general population
who meet the CCES survey criteria, i.e., elibible voters living in
one of the 50 states or Washington DC.  (Note: STATEFIP codes have gaps, Wyoming has STATEFIP code 56).
Therefore,  we drop all records for individuals under the age of 18 and non-citizens, or citizens not
living in the 50 states or DC.
```{r}
# using the level name here
table(as_factor(acs_2018_subset_tbl$CITIZEN))
citizen = as_factor(acs_2018_subset_tbl$CITIZEN) != "Not a citizen"
# using the column value
over_18 = acs_2018_subset_tbl$AGE > 17
state_or_DC = acs_2018_subset_tbl$STATEFIP < 57  
acs_2018_subset_tbl = acs_2018_subset_tbl[(over_18 & citizen & state_or_DC) , ]  
```

The metadata is in the object `acs_2018_subset_ddi`.
Working with the `tbl_df` object is awkward, as the MRP-KIT API expects to work with simple dataframes.
Therefore we create a data.frame object which contains only the columns of interest as factors,
excepting "AGE" which is count, not categorical, data.
```{r}
acs_2018_subset <- data.frame(
    "STATEFIP" = droplevels(as_factor(acs_2018_subset_tbl$STATEFIP)),
    "SEX" = droplevels(as_factor(acs_2018_subset_tbl$SEX)),
    "AGE" = zap_labels(acs_2018_subset_tbl$AGE),
    "MARST" = droplevels(as_factor(acs_2018_subset_tbl$MARST)),
    "RACE" = droplevels(as_factor(acs_2018_subset_tbl$RACE)),
    "HISPAN" = droplevels(as_factor(acs_2018_subset_tbl$HISPAN)),
    "CITIZEN" = droplevels(as_factor(acs_2018_subset_tbl$CITIZEN)),
    "EDUC" = droplevels(as_factor(acs_2018_subset_tbl$EDUC))
)
str(acs_2018_subset, max.level=2, list.len=12, give.attr=FALSE)
```
...
```{r}
library(mrpkit)
levels(cces_2018_subset$gender)
levels(acs_2018_subset$SEX)
...
```{r}
cat("CCES:", levels(cces_2018_subset$inputstate)[1:5], "\n", sep=", ");
cat("ACS:", levels(acs_2018_subset$STATEFIP)[1:5], "\n", sep=", ");
table(levels(cces_2018_subset$inputstate) == levels(acs_2018_subset$STATEFIP))
```
...
```{r}
state_names <- levels(cces_2018_subset$inputstate)
state_values <- levels(acs_2018_subset$STATEFIP)
# create named list in two steps
mapping <- as.list(state_values)
names(mapping) <- state_names 
mapping[1:3]
# one-liner using `setNames`
mapping = as.list(setNames(state_values, state_names))
mapping[1:3]
```
...
```{r}
state_mapping = as.list(setNames(state_values, state_names))
...
```{r}
levels(cces_2018_subset$educ)
levels(acs_2018_subset$EDUC)
```

For the ACS data, a possible label is "3 years of college" however
in the ACS subset used here, this label isn't used.
If it were used, we would have a choice between mapping it to CCES
labels "Some college" or "2-year", on the rational that if the
 goal was a 4-year college degree, and that it wasn't obtained.
...
```{r}
levels(cces_2018_subset$race)
levels(acs_2018_subset$RACE)
```
...
```{r}
levels(cces_2018_subset$hispan)
levels(acs_2018_subset$HISPAN)
```
...
acs_2018_subset$race2 = acs_race2
```
...
length(which(cces_2018_subset$race == "Hispanic" | cces_2018_subset$hispan == "Yes"))
# create column which adjusts race labels, add to dataframe
...
```{r}
levels(cces_2018_subset$marstat)
levels(acs_2018_subset$MARST)
```
...
acs_marst2 <- acs_2018_subset$MARST
...
acs_marst2 <- droplevels(acs_marst2)  # unused levels not allowed
# add to acs_2018_subset data.frame
acs_2018_subset$marst2 = acs_marst2
```
...
## Measurement recorded in different units and not binned:  brithyear and age
...
```{r}
# use R `cut` functon, choose breakpoints, labels
...
# add to CCES
cces_2018_subset$birthyr_cohort = cut(cces_2018_subset$birthyr, cohort_breaks, cohort_labels)
attr(cces_2018_subset, "var.labels")[['birthyr_cohort']] <- "Birthyear cohort"

# transform ACS age to birthyr, then add to ACS
acs_2018_subset$birthyr = 2018 - acs_2018_subset$AGE
acs_2018_subset$birthyr_cohort = cut(acs_2018_subset$birthyr, cohort_breaks, cohort_labels)
```

This is a straightforward 1:1 mapping
```{r}
birthyr_cohort_mapping = as.list(setNames(cohort_labels, cohort_labels))
q_birthyr <- SurveyQuestion$new(name = "birthyear",
	col_names = c("birthyr_cohort", "birthyr_cohort"),
	values_map= birthyr_cohort_mapping)
q_birthyr$name()
q_birthyr$col_names()
head(q_birthyr$values())
```
...
`q_sex` (2 values), `q_state` (51 values), `q_educ` (6 values), `q_race` (8 values), `q_marstat` (5 values), `q_birthyr` (many possible values - e.g. 4 or 7).
...
The components of an MRP-KIT mapping are the survey data, the target population data,
and one or more mappings between demographics of interest.  In the previous sections
we assembled the sample and target population datasets and defined the SurveyQuestion
objects for to each demographic, adjusting the datasets accordingly.
Next we create SurveyData objects for each dataset.

A SurveyData object records the contents of a dataset in terms of questions and
responses.  Every column is (potentially) a question, and the responses are the set
of category labels for that column.  (*what about continuous variables?*)
The question and responses are name-value pair lists.
For the question list, the name is the dataframe column name and the value is the description of the variable or the survey question asked.
For the response list, the name is also the dataframe column name and the value is the set of possible responses.

For the CCES, the dataframe column descriptions are lists in the attrbute "var.labels".
As before, we create this list programatically using the `as.list(setNames(tags,values))` one-liner.
```{r}
cces_2018_questions <- as.list(setNames(attr(cces_2018_subset,"var.labels"), names(cces_2018_subset)))
head(cces_2018_questions)
```
For the ACS, we use function `ipumsr::ipums_var_label` to get a short description of the variable
from the metadata in the `acs_2018_subset_tbl` object.
```{r}
acs_2018_var_labels <- c()
for (name in names(acs_2018_subset)) {
    acs_2018_var_labels <- c(acs_2018_var_labels, ipums_var_label(acs_2018_subset_tbl[[name]]))
}
acs_2018_questions <- as.list(setNames(acs_2018_var_labels, names(acs_2018_subset)))
head(acs_2018_questions)
```

A response list is a list of name-value pairs which map the column name to the set of
possible response values.
We create this list by using R's `apply` function.
```{r}
cces_2018_responses <- apply(cces_2018_subset, 2, function(x) levels(as.factor(x)))
head(cces_2018_responses)
acs_2018_responses <- apply(acs_2018_subset, 2, function(x) levels(as.factor(x)))
head(acs_2018_responses)
```

We use these lists to create SurveyData objects for both the CCES and ACS.
```{r}
cces_2018_data <- SurveyData$new(cces_2018_subset, cces_2018_questions, cces_2018_responses)
cces_2018_data
acs_2018_data <- SurveyData$new(acs_2018_subset, acs_2018_questions, acs_2018_responses)
acs_2018_data
```
Now we can create the mapping object from the CCES to the ACS.
The `print` function displays the mappings between the survey data and target population data.
```{r}
cces_acs_mapping <- SurveyMap$new(cces_2018_data, acs_2018_data, q_sex, q_state, q_educ, q_race, q_marstat, q_birthyr)
print(cces_acs_mapping)
```
The `validate` function identifies mapped/unmapped columns and validates the mappings defined by the SurveyQuestion objects
against the levels defined on the datasets.
```{r}
cces_acs_mapping$validate()
```

The `tabulate` function creates the post-stratification table.
```{r}
cces_acs_mapping$tabulate()
```

# MRP Part One:  Multi-level regression

Given the survey data, we fit a model to the survey data, using our favorite R package of choice.

# MRP Part Two:  Post-stratification
