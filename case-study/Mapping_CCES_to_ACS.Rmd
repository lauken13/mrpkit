...
Therefore,  we drop all records for individuals under the age of 18,
non-citizens, or residing outside of the 50 states or DC.
...
state_or_DC = acs_2018_subset$STATEFIP < 52
acs_2018_subset = acs_2018_subset[(over_18 & citizen & state_or_DC) , ]  
```
...
# ACS dataframe is haven_labelled, treat as factor, only keep states and DC
STATEFIP_levels <- levels(as_factor(acs_2018_subset$STATEFIP))[1:51]
cat("CCES:", inputstate[1:5], sep=", ");
cat("ACS:", STATEFIP_levels[1:5], sep=", ")
table(inputstate == STATEFIP_levels)
```
...
mapping <- as.list(STATEFIP_levels)
names(mapping) <- toupper(inputstate)
mapping[1:3]
# combine the two steps into a one-liner (lowercase)
mapping = as.list(setNames(STATEFIP_levels, tolower(inputstate)))
mapping[1:3]
```
...
```{r}
state_mapping = as.list(setNames(STATEFIP_levels, inputstate))
...
acs_2018_subset$birthyr_cohort = to_labelled(cut(acs_2018_subset$birthyr, cohort_breaks, cohort_labels))
```
...
```
For the ACS, we use the function `ipumsr::ipums_var_label` to get a short description of the variable.
```{r}
