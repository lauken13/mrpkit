...
A SurveyData object requires a dataframe, a question list, and a response list.
A question list is a list of tag-value pairs where the tag (name) is the dataframe column name and the value is a description of corresponding survey question.
For the CCES, the dataframe column descriptions are lists in the attrbute "var.labels".
As before, we create this list programatically using the `as.list(setNames(tags,values))` trick.
```{r, EVAL=FALSE}
...
from the metadata in the DDI object.
```{r, EVAL=FALSE}
acs_2018_var_labels <- c()
for (name in names(acs_2018_subset)) {
    acs_2018_var_labels <- c(acs_2018_var_labels, ipums_var_label(acs_2018_subset[[name]]))
}
...
We can create this list using a for loop, or by using R's `apply` function.
```{r, EVAL=FALSE}
# create response list using a for loop
cces_2018_responses <- list()
for (name in names(cces_2018_subset)) {
    cces_2018_responses[[name]] <- levels(as.factor(cces_2018_subset[[name]]))
}
head(cces_2018_responses)

acs_2018_responses <- list()
for (name in names(acs_2018_subset)) {
    acs_2018_responses[[name]] <- levels(as_factor(acs_2018_subset[[name]]))
}
acs_2018_responses[["MARST"]]
acs_2018_responses[["race2"]]
acs_2018_responses[["birthyr_cohort"]]

# cces_2018_subset is a dataframe; R's `apply` function would also work
# cces_2018_responses <- apply(cces_2018_subset, 2, function(x) levels(as.factor(x)))
# if we coerce acs_2018_subset to dataframe, can do the same
# acs_2018_responses <- apply(as.data.frame(cces_2018_subset), 2, function(x) levels(as.factor(x)))
```
...
```{r, EVAL=FALSE}
cces_acs_mapping <- SurveyMap$new(cces_2018_data, acs_2018_data, q_sex, q_state, q_race, q_educ)
# poststrat table size: 2 * 51 * 8 * 6
```
