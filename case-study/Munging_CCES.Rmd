...
The attribute `names` corresponds to the dataframe column names and
the attribute `val.labels` provides a short description of that columns.
For questions which have categorical responses, the attribute `data.label`
...
# don't allow missing demographics
keep_rows <- complete.cases(cces_2018[, demo_cols])
...
reasoning that the goal of a 4-year college degree wasn't obtained.
The choice depends on both an understanding of the survey data and the particular goals
of your analysis.
MRP-KIT can only provide the mechanism for recording and carrying out the mapping.
```{r}
educ_mapping = list(
    "No HS" = "N/A or no schooling",
    "No HS" = "Nursery school to grade 4",
...
Mapping both ACS categories to CCES category "Married" is a one-to-many mapping
which is allowed by MRP-KIT.
The CCES has category "Domestic / civil partnership",
for which there is no corresponding ACS category.
Mapping both partnerships and "Never married" to the ACS category "Never married/single"
is a many-to-one mapping from CCES to ACS, also allowed by MRP-KIT.
However, if we map CES category  "Domestic / civil partnership"
to either ACS category "Married, spouse present" or "Married, spouse absent",
we create a many-to-many mapping.
...
```
Here we propose to add yet another column to the ACS which conflates "Married, spouse present" and "Married, spouse absent".
...
same sex marriages and civil unions and use this information to code new variable "marst2".]
...
We have created 5 questions:  `q_sex`, `q_state`, `q_educ`, `q_race`, `q_marstat`.
This will allow us to slice and dice our surveys to a ridiculous level of granularity.
Excelsior!
